...
//        return Objects.equals(scanType, "C") ?  ("aeiou").indexOf($) == -1: ("aeiou").indexOf($) != -1;
        //simplified version of intellijs below:
        ...
        for (char aChar : chars) {
            if (genScanCondition(aChar, scanType)) {
                ...
                if (lettersMap.containsKey(consonantChar)) { // check if key is present
                    ...
                    lettersMap.put(consonantChar, 1); //if a value to the appropriate consonant does not exist then start by 1
                }
            }
        }
        return lettersMap;
...
//        char[] chars = input.toCharArray();
//        for (char aChar : chars) {
//            if ((("aeiou").indexOf(aChar)) != -1) { //basically the same like above just reversing the condition
